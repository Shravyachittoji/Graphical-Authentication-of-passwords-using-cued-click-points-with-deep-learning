...
class HybridModel(nn.Module):
    def __init__(self, num_classes=10):
        ...
        # Enhanced fusion layer to better combine features from both models
        self.fusion = nn.Sequential(
            nn.Linear(num_classes * 2, 1024),
            nn.LayerNorm(1024),
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(1024, 512),
    ...
    def forward(self, x):
        ...
        drn_probs = F.softmax(drn_out, dim=1)
        vit_probs = F.softmax(vit_out, dim=1)
        
        # Concatenate the outputs
        combined = torch.cat((drn_probs, vit_probs), dim=1)
...
def compare_models(dataset_path, epochs=5, num_images=3):
    ...
    try:
        ...
        # Initialize trend arrays with initial values based on actual metrics
        drn_irr_trend = [drn_metrics['accuracy']]  # Use actual accuracy
        drn_pds_trend = [drn_metrics['f1']]  # Use actual F1 score
        vit_irr_trend = [vit_metrics['accuracy']]  # Use actual accuracy
        vit_pds_trend = [vit_metrics['f1']]  # Use actual F1 score
        hybrid_irr_trend = [hybrid_metrics['accuracy']]  # Use actual accuracy
        hybrid_pds_trend = [hybrid_metrics['f1']]  # Use actual F1 score
        existing_gpass_irr_trend = [0.45]  # Keep traditional baseline
        existing_gpass_pds_trend = [0.40]  # Keep traditional baseline
        ...
        # Calculate standard deviations based on actual metrics
        drn_irr_std = np.std(drn_metrics['precision_history'])
        drn_pds_std = np.std(drn_metrics['f1_history'])
        vit_irr_std = np.std(vit_metrics['precision_history'])
        vit_pds_std = np.std(vit_metrics['f1_history'])
        hybrid_irr_std = np.std(hybrid_metrics['precision_history'])
        hybrid_pds_std = np.std(hybrid_metrics['f1_history'])
        existing_gpass_irr_std = 0.171  # Keep traditional baseline
        existing_gpass_pds_std = 0.085  # Keep traditional baseline
...
def train_model(model, dataset_path, epochs):
    """Train a model and return training metrics, including confusion matrix and ROC/AUC."""
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (images, labels) in enumerate(dataloader):
            ...
            # For ROC, get probability for class 1 (if binary) or all classes
        ...
        if len(predictions) > 0 and len(true_labels) > 0:
            ...
            metrics['precision_history'].append(precision)
            metrics['recall_history'].append(recall)
            metrics['f1_history'].append(f1)
    ...
    if all_true_labels and all_predictions:
        metrics['confusion_matrix'] = confusion_matrix(all_true_labels, all_predictions)
    # ROC curve and AUC (only for binary or one-vs-rest)
    try:
        n_classes = np.unique(all_true_labels).size
        if n_classes == 2:
            # Binary: use prob for class 1
            y_true = np.array(all_true_labels)
            y_score = np.array(all_pred_probs)[:, 1]
            ...
            y_true = label_binarize(np.array(all_true_labels), classes=np.arange(n_classes))
